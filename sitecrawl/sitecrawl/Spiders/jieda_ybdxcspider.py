...
class A(threading.Thread):
    ...
    def run(self):
        ...
        for date in ywdateend_list:
                item = {}
                datesta = "{}-{}".format(date[0], date[1])
                dateend = "{}-{}-{}".format(date[0], date[1], date[2])
                item['date'] = "{}-{}".format(date[0], date[1])
                yw_data = {
                    'dtid': dtid_code,
                    'cmd_0': 'onChange',
                ...
                sid += 1
                yw_info = requests.post(zk_url, data=yw_data, headers=headers).text
                yw_info = yw_info.replace(r'\"', '"')
                siteid_body = r'"loginSiteId":(.*?),'
                siteid_pattern = re.compile(siteid_body)
                siteid = re.findall(siteid_pattern, yw_info)[0]
                print(siteid)

                sqlid_body = '"sqlAuthedUserId":(.*?),'
                sqlid_pattern = re.compile(sqlid_body)
                sqlid = re.findall(sqlid_pattern, yw_info)[0]
                print(sqlid)

                GMT_FORMAT = '%a %b %d %Y %H:%M:%S GMT+0800 （中国标准时间）'
                date_parm = datetime.datetime.now().strftime(GMT_FORMAT)
                parm = '{"ewbStatus":"1","maxFreightCharge":null,"receiveCountyId":null,"minFreightCharge":null,"moduleType":1,"maxCodCharge":null,"ewbNoSendFlag":"","minWeight":null,"notSendSiteId":' + siteid + ',"maxWeight":null,"minCodCharge":null,"loginSiteId":' + siteid + ',"productNameId":null,"receiveProvinceId":null,"sendLook":1,"sqlAuthedUserId":' + sqlid + ',"blSign":"","receiveCityId":null,"endEwbDate":"' + dateend + ' 23:59:59","sqlAuthedFlag":1,"startEwbDate":"' + datesta + '-01 00:00:00","ewbNoTransferFlag":""}'
                total_url = 'http://sxne.sxjdfreight.com/ewbSendFeeSumServlet?reshSumContentDate={}&mapCondition={}'.format(
                    date_parm, parm)
                res = requests.get(total_url).text
                obj = json.loads(res)
                item['ps_'] = obj['sendTotalVotes']
                item['js_'] = obj['sendTotalPrice']
                item['jswidth'] = obj['calcWeight']
                item['zzjswidth'] = obj['transferCalcWeight']
                item['sjwidth'] = obj['weight']
                item['tj_'] = obj['vol']
                item['dfk'] = obj['arrivePaymentCharge']
                item['xj_'] = obj['spotPaymentCharge']
                item['yj_'] = obj['monthlyPaymentCharge']
                item['yfhj'] = obj['freightCharge']
                item['sfhj'] = obj['totalAmount']
                item['cbhj'] = obj['costTotal']
                item['ml_'] = obj['siteGrossMargin']
                yw_list.append(item)
        gLock.acquire()
        item_all['ztList'] = yw_list
        gLock.release()
...
class B(threading.Thread):
    ...
    def run(self):
        ...
        for date in ywdateend_list:
            ...
            date_parm = datetime.datetime.now().strftime(GMT_FORMAT)
            # print(date_parm)
            parm = '{"ewbStatus":"1","maxFreightCharge":null,"sendSiteId":' + siteid + ',"receiveCountyId":null,"minFreightCharge":null,"moduleType":1,"maxCodCharge":null,"ewbNoSendFlag":"","minWeight":null,"maxWeight":null,"minCodCharge":null,"loginSiteId":' + siteid + ',"productNameId":null,"receiveProvinceId":null,"sendLook":1,"sqlAuthedUserId":' + sqlid + ',"blSign":"","receiveCityId":null,"endEwbDate":"' + dateend + ' 23:59:59","sqlAuthedFlag":1,"startEwbDate":"' + datesta + '-01 00:00:00","ewbNoTransferFlag":""}'
            total_url = 'http://sxne.sxjdfreight.com/ewbSendFeeSumServlet?reshSumContentDate={}&mapCondition={}'.format(
                date_parm, parm)
            # print(total_url)
            res = requests.get(total_url).text
