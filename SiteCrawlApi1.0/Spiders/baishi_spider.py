...
def baishi_spider(username,password):
    ...
    comimg.screenshot('code_baishi.png')
    # try:
    ...
    if json.loads(dic)["code"] == "200":
        ...
        # 网点基础信息
        item = {}
        #base_info = json.loads(requests.post(baseinfo_url, data=data, headers=headers).text)
        #item['siteName'] = base_info['vo']['name']
        #item['principal'] = base_info['vo']['principal']
        #item['location'] = base_info['vo']['location']
        # print(item)
        ...
        #date_list6 = []
        #for n in range(1, 13):
        #    d = cal.monthrange(year - 2, n)
        #    date_list3.append(FORMAT % (year - 2, n, 1) + " 00:00:00")
        #date_list6 = []
        ...
        pyload = {
        "currentPage":1,
        "dateEnd":"{}-{}-1 23:59:59".format(year,month-1),
        "dateStart":"{}-{}-1 00:00:00".format(year-1,month),
        "res":"SUCCESS",
        "pageSize":99999,
        }
        recharge_r = requests.post(recharge_url,data = json.dumps(pyload),headers=acc_headers).text
        ...
        # # 网点投诉情况
        # sits_r = json.loads(requests.post(ts_url, headers=acc_headers), d)
        # print(item)
        driver.quit()
        return item
    else:
